...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const canvas = document.createElement('canvas');
      const gl = canvas.getContext('webgl2', { antialias, alpha: true }) as WebGL2RenderingContext | null;
      if (!gl) {
        ...
        const noiseEffect = new Effect(
          ...
          `uniform float uTime; 
           ...
           void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) { 
             float n = hash(floor(uv * vec2(1920.0, 108.0)) + floor(uTime * 60.0)); 
      ...
      // Set up composer size if using post-processing
      if (composer) {
        composer.setSize(renderer.domElement.width, renderer.domElement.height);
      }

      // Handle pointer events for interactive effects
      ...
      // Update existing Three.js context with new props
      const t = threeRef.current!;
    ...
    // Store current config for comparison on next update
    prevConfigRef.current = cfg;

    // Cleanup function
    return () => {
      // Skip cleanup if we're just updating props
  ...
  // Track component visibility for auto-pause
  useEffect(() => {
